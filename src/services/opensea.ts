...
export const getBestOfferByNFT = async (nftId: string) => {
  const bestOffer = await fetchFromOpenSea(`https://api.opensea.io/api/v2/offers/collection/${collectionId}/nfts/${nftId}/best`);
  return bestOffer;
};

export const getBestListingsByCollection = async () => {
  const bestListings = await fetchFromOpenSea(`https://api.opensea.io/api/v2/listings/collection/${collectionId}/best`);
  return bestListings;
};